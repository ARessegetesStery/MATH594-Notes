...
\begin{proof}
    ...
    \begin{lemma}\label{lem: simple ext of separable elem gives nonzero trace}
        If $K \ext K' = K(\alpha)$ where $\alpha$ is separable over $K$. Then $\Tr_{K'/K} \neq 0$.
    \end{lemma}
    \nogap
    \begin{lemma}\label{lem: simple ext of nilpotent elem gives zero trace}
        If $K \ext K' = K(\alpha)$ where $\fchar(K) = p$, $\alpha \notin K$, $\alpha^p \in K$. Then $\Tr_{K'/K} = 0$.
    \end{lemma}
    These two implies the theorem:
    \begin{itemize}
        \item If $L/K$ is not separable, then there exists $\alpha \in L$ that is not separable over $K$. 
    \end{itemize}
\end{proof}
